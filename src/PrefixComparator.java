...
public class    PrefixComparator implements Comparator<Term> {
    ...
    public int compare(Term v, Term w) {
        ...
        int mySize = myPrefixSize;
        String vWord = v.getWord();
        String wWord = w.getWord();
        if (vWord.length() < myPrefixSize || wWord.length() < myPrefixSize) { //if one or both words have size less than myPrefixSize
            mySize = Math.min(vWord.length(), wWord.length());
        }
        int index = 0;
        while (index != mySize) {
            if (vWord.charAt(index) < wWord.charAt(index)) {
                return -1;
            }
            if (vWord.charAt(index) > wWord.charAt(index)) {
                return 1; 
        ...
        if(wWord.length()<vWord.length() && (myPrefixSize > vWord.length() || myPrefixSize > wWord.length())){
            return 1;
        }
        if(wWord.length()>vWord.length() && (myPrefixSize > vWord.length() || myPrefixSize > wWord.length())){
            return -1;
